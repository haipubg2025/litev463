...
export default function NpcUpdateModal({ npc, npcIndex, onClose, onApply }: NpcUpdateModalProps) {
  ...
  const isFieldAllowed = (key: string): boolean => {
    ...
    // Các trường hệ thống / gameplay cốt lõi luôn luôn được cho phép bất kể chế độ bảng nào
    const systemFields = ["id", "name", "role", "avatar", "status", "statusdata", "location", "currentlocation", "partylist", "objectives"];
    if (systemFields.includes(keyLower)) {
      return true;
    }

    if (isCustomMode) {
      // Trong chế độ Custom, chỉ cho phép các trường do người dùng tự định nghĩa trong bảng tùy chỉnh
      return customFields.some((f: any) => f.id.toLowerCase() === keyLower);
    } else {
      ...
      return defaultFields.includes(keyLower);
    }
  };
  ...
  const [pending, setPending] = React.useState<any>(() => {
    ...
    const filtered: any = {};
    Object.entries(rawPending).forEach(([key, val]) => {
        ...
        <div id="npc-update-scroll-container" className="flex-1 overflow-y-auto flex flex-col p-6 gap-8">
          <div className="max-w-7xl mx-auto w-full flex flex-col gap-8 pb-20">
            {Object.keys(pending).map((key) => {
              ...
              const isArray = (Array.isArray(origVal) || Array.isArray(pending[key])) && 
                              !(typeof origVal === 'string' || typeof pending[key] === 'string');
              const isObject = !isArray && 
                               ((typeof origVal === 'object' && origVal !== null) || (typeof pending[key] === 'object' && pending[key] !== null)) &&
                               !(typeof origVal === 'string' || typeof pending[key] === 'string');
              ...
              return (
                <div key={key} className={`flex flex-col md:flex-row gap-6 ${viewMode !== 'both' ? 'md:flex-col' : ''}`}>
                  ...
                  <div className={`flex-1 p-5 rounded-2xl border relative focus-within:ring-2 ring-green-500/50 shadow-sm ${isDark ? 'bg-green-900/10 border-green-500/30' : 'bg-green-50 border-green-200'} flex flex-col`}>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {typeof npc[key] === 'string' && typeof pending[key] === 'string' && (
                          <button 
                            onClick={() => handleChange(key, npc[key] + '\n\n' + pending[key])} 
                    ...
                    <div className="flex-1">
                      {isArray ? (
                        <ArrayEditor items={pending[key] || []} originalItems={npc[key] || []} isDark={isDark} onChange={(val) => handleChange(key, val)} showOnlyChanges={true} />
                      ) : isObject ? (
                        <ObjectEditor obj={pending[key] || {}} originalObj={npc[key] || {}} isDark={isDark} onChange={(val) => handleChange(key, val)} showOnlyChanges={true} />
                      ) : (
                        <div className="flex flex-col gap-2 w-full">
                          ...
                          />
                          <DiffPreview oldVal={npc[key]} newVal={pending[key]} isDark={isDark} />
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {Object.keys(pending).filter(key => JSON.stringify(pending[key]) !== JSON.stringify(npc[key]) && !['location', 'currentlocation', 'status', 'statusdata'].includes(key.trim().toLowerCase())).length === 0 && (
